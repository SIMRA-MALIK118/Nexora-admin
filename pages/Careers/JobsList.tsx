...
import { Plus, MapPin, Briefcase, Clock, Trash2, X, Loader2, CheckCircle2, ImageIcon } from 'lucide-react';
import { Job } from '../../types';
import { JobService } from '../../services/storageService';

const INITIAL_JOBS: Job[] = [
  { id: '1', role: 'Senior React Engineer', location: 'Remote', type: 'Full-time', status: 'Open', imageUrl: 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?w=400&h=200&fit=crop' },
  { id: '2', role: 'UI/UX Designer', location: 'London, UK', type: 'Contract', status: 'Open', imageUrl: 'https://images.unsplash.com/photo-1586717791821-3f44a563eb4c?w=400&h=200&fit=crop' },
];
...
const JobsList: React.FC = () => {
  ...
  useEffect(() => {
    setJobs(JobService.getAll(INITIAL_JOBS));
  }, []);
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-full md:max-w-md bg-white shadow-2xl z-[70] p-6 md:p-8 animate-in slide-in-from-right duration-300 overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-xl font-bold text-gray-900">New Job Listing</h3>
            ...
            <form onSubmit={handleAddJob} className="space-y-6">
              ...
              <div>
                ...
                <input required type="text" value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} className="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-black outline-none font-medium" placeholder="e.g. Remote or City, Country" />
              </div>
              ...
              <div className="pt-8">
                <button disabled={isSubmitting} type="submit" className="w-full bg-[#0d0d0d] text-white py-4 rounded-xl font-bold hover:bg-black/90 transition-all shadow-xl flex items-center justify-center gap-2">
                  ...
                  {isSubmitting ? 'Posting...' : 'Save Job Posting'}
                </button>
              </div>
            </form>
          </div>
